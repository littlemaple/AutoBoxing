...
wysiHelpers = {
	...
	getImageToolbarHtml: function() {
		var locale = $.fn.wysihtml5.locale["en"];
		return "<div class='bootstrap-wysihtml5-insert-image-modal modal hide fade'>" +
...
bootWysiOverrides = {
	initInsertImage: function(toolbar) {
		...
		toolbar.children().last().html(wysiHelpers.getImageToolbarHtml());
		var self = this;
			...
			toolbar.find('a[data-wysihtml5-command=insertImage]').click(function() {
				...
				if (!activeButton) {
					...
					$('#file1').change(function() {
						$(this).uploadimage('', {'wysihtml5':'upload'}, function(res) {
							if(res.status) {
								if (chooser.find('tr').length > 3) {
									chooser.find('tr').first().remove();
								}
